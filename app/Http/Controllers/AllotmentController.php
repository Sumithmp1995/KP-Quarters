...
class AllotmentController extends Controller
{
    ...
    {  
        ...
        if ($data['class'] == 'LSQ') {
            ...
            // FETCH the COUNT of Lower Subordinate Executive Applicants
            ...
            //error
            if ((!empty($executiveLs['lsq_quarters_count'] - $executiveLs['executive_ls_count']))) {
            ...
            if (!empty($executiveUs['usq_quarters_count'] - $executiveUs['executive_us_count'])) {
                $allottee = Allottee::where('unitHead_id', auth()->user()->unitHead_id)
                    ->where('quarters_id', null)
                    ->where('applicant_type', 'MINISTERIAL')
                    ->where('type', 'USQ')
    ...
    public function confirmWilling($id)
    {
        ...
        $user = User::where('unitHead_id', auth()->user()->unitHead_id)
            ->where('role', '2')->first();
        $path = "/preview_allot_quarters/$occupant->id";
    ...
    public function allotQuartersPreview($occupant_id)
    {
        $occupant = Occupant::where('occupants.id', $occupant_id)
            ->join('applicants', 'applicants.id', '=',  'occupants.applicant_id')
    ...
    public function allotQuarters($allottee_id)
    {
        ...
        $message =  'click to view allot proceedings';
